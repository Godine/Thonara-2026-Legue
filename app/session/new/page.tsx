...
export default function NewSessionPage() {
  ...
  const createSession = async () => {
    ...
    if (playerErr || !players) {
      setError('Could not load players. Check Supabase setup.')
      setLoading(false)
      return
    }
    ...
    const { data: session, error: sessionErr } = await supabase
      .from('sessions')
      .insert({ date, created_by: user.id })
      .select('id')
      .single()
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 animate-fade-in">
      {/* Header */}
      <div className="mb-6">
        <p className="font-body text-xs tracking-widest uppercase text-pool-chalk-dim mb-1">
          New session
        </p>
          ...
          {GAME_SCHEDULE.map(g => {
            ...
            return (
              <div key={g.gameNumber} className="flex items-center px-4 py-3 gap-3">
                <span className="font-heading text-sm text-pool-chalk-dim w-6">
                  {g.gameNumber}
                </span>
                {/* Player 1 */}
